...
const EditPendingBookings = () => {
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            console.log('current update booking=',res);
            if (res.data.status == true) {
                swalAlert.success(res.data.message);
                setTimeout(() => {
                    router.push('/librarian/booking/currentlybookFolder/currently-book')
                }, 2000);
            }
    ...
    return (
        <Layout>
            <div className='bg-slate-300 h-auto p-0 m-0'>
                <h1 className='text-center pt-4 font-semibold text-lg text-cyan-800'>Update Pedning Booking</h1>
                <div className='w-11/12 md:w-1/2 mx-auto mt-4 bg-slate-200 shadow-lg p-10 rounded-lg'>
                    <Form onFinish={onSubmit} layout='vertical'>
                        ...
                        <Button type='primary' htmlType='submit'>Update Pedning Booking</Button>
                    </Form>
                </div>
            </div>
        </Layout>
...
export default EditPendingBookings;