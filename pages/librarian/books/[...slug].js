...
const EditBook = () => {
    ...
    const description = router.query.slug[3];
    const type = router.query.slug[4];
    const quantity = router.query.slug[5];
    const bookId = router.query.slug[6];
    console.log(bookTitle, authorName, pulisherName, description, type, quantity, bookId);
    const onSubmit = async (values) => {
        try {
            ...
            const res = await axios.patch(`http://localhost:8080/books/update/${bookId}`, values, config);
    ...
    return (
        <Layout>
            <div className='bg-slate-300 h-screen p-0 m-0'>
                <h1 className='text-center pt-6 font-semibold text-lg text-cyan-800'>Update Book</h1>
                <div className='w-11/12 md:w-1/2 mx-auto mt-8 bg-slate-200 shadow-lg p-10 rounded-lg'>
                    <Form onFinish={onSubmit} layout='vertical'>
                        ...
                        <div className='md:grid md:grid-cols-2 md:gap-1'>
                            ...
                            <Form.Item label='Quantity' name="quantity" hasFeedback>
