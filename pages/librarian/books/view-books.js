...
import { deleteBooks } from '../../../components/helper/delete';
...
const ViewAllBooks = () => {
    ...
    const editHandler = (bookTitle, authorName, pulisherName, description, type, quantity, id) => {
        //console.log(bookTitle, authorName, pulisherName, description, type, quantity, id);
        router.push(`/librarian/books/${bookTitle}/${authorName}/${pulisherName}/${description}/${type}/${quantity}/${id}`)
    }
    ...
    const column = [
        ...
        { dataField: 'quantity', headerName: 'Quantity' },
        {
            dataField: '_id', headerName: 'Action', formatter: (_id, data) => (
                <div>
                    <button onClick={() => editHandler(data.bookTitle, data.authorName, data.pulisherName, data.description, data.type, data.quantity, _id)} className='editBtn mr-2 tracking-wide hover:text-green-700 hover:font-bold hover:text-xl'>Edit</button>
