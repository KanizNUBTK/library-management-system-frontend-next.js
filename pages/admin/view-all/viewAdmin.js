...
import { deleteLibrary, deleteUser } from '../../../components/helper/delete';
...
const ViewAdmin = () => {
    ...
    useEffect(() => {
        async function getData() {
            try {
                ...
                const result = await axios.get(`http://localhost:8080/user/get-filtered-data?role=admin`, config);
                console.log("result admin=", result.data.data);
    ...
    return (
        <Layout>
            <h1 className='text-center font-bold text-2xl mt-4 text-cyan-600'>Admin List</h1>
